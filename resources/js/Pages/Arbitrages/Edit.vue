<template>
  <div>
    ...
    <h1 class="mb-8 text-3xl font-bold">
      <Link class="text-indigo-400 hover:text-indigo-600" href="/arbitrages">Arbitrage</Link>
    ...
    <div class="max-w-3xl bg-white rounded-md shadow overflow-hidden">
      <form @submit.prevent="update">
        ...
        <div class="flex items-center justify-end px-8 py-4 bg-gray-50 border-t border-gray-100">
          <loading-button :loading="form.processing" class="btn-indigo" type="submit">Edit Arbitrage</loading-button>
        </div>
      </form>
    </div>
  </div>
...
export default {
  ...
  props: {
    coins: Array,
    coinArbitrage: Object,
  },
  remember: 'form',
  data() {
    return {
      form: this.$inertia.form({
        coin_one_id: this.coinArbitrage.coin_one_id,
        coin_one_price: this.coinArbitrage.coin_one_price,
        coin_two_id: this.coinArbitrage.coin_two_id,
        coin_two_price: this.coinArbitrage.coin_two_price,
        coin_three_id: this.coinArbitrage.coin_three_id,
        coin_three_price: this.coinArbitrage.coin_three_price,
        profit: this.coinArbitrage.profit,
        capital: this.coinArbitrage.capital,
        test_mode: this.coinArbitrage.test_mode ? 1 : 0,
        enabled: this.coinArbitrage.enabled ? 1 : 0,
      }),
    }
  },
  methods: {
    update() {
      this.form.put(`/arbitrages/${this.coinArbitrage.id}`)
    },
  },
}
</script>
